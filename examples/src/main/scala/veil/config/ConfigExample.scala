...
/** An example showing how to use the Config typeclass to load configuration
  * from JSON files.
  *
  ...
  *
  * To run this example:
  *   1. Create a config.json file in your project root (see example below) 2.
  *      Run: sbt "examples/runMain veil.config.ConfigExample"
  *
  * Example config.json:
  * {{{
  * {
  ...
  def main(args: Array[String]): Unit = {
    ...
    config.fromFile("config.json") match {
      case scala.util.Success(appConfig) =>
        ...
        println(
          s"  Database: ${appConfig.database.host}:${appConfig.database.port}/${appConfig.database.name}"
        )
