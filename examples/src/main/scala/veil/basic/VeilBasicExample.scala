...
/** A basic example showing how to use Veil to read environment variables.
  ...
  *
  * To run this example:
  * 1. Create a .env file in your project root with some variables
  * 2. Run: sbt "examples/runMain veil.basic.VeilBasicExample"
  *
  ...
  */
object VeilBasicExample extends App {
  ...
  println(s"Port (checking PORT, APP_PORT, SERVER_PORT): ${port.getOrElse("8080")}")
  println()
