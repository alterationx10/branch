...
/** An example showing how to expose application metrics via JMX.
  ...
  *   - Histograms: Distribution/summary data (average response time,
  *     percentiles)
  *
  ...
  *
  * To run this example:
  *   1. Run: sbt "examples/runMain blammo.metrics.MetricsExample" 2. In another
  *      terminal, run: jconsole 3. Connect to the running process 4. Navigate
  *      to MBeans -> com.example.myapp -> type=ApiService -> Metrics 5. Watch
  *      the metrics update in real-time
  *
  ...
  def main(args: Array[String]): Unit = {
    ...
    for (i <- 1 to 20) {
      ...
      if (i % 5 == 0) {
        ...
        metrics
          .getCounter("TotalRequests")
          .foreach(c => println(s"  Total Requests: $c"))
        metrics
          .getCounter("TotalErrors")
          .foreach(e => println(s"  Total Errors: $e"))
        metrics
          .getHistogram("AvgResponseTime")
          .foreach(rt => println(f"  Avg Response Time: $rt%.2f ms"))
        metrics
          .getGauge("QueueSize")
          .foreach(qs => println(s"  Queue Size: $qs"))
        println()
      }
    }
    ...
    println("(Open JConsole now to inspect final metrics)")
