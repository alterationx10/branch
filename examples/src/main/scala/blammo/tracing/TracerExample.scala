...
/** An example showing distributed tracing via structured logging.
  ...
  *
  * To run this example: sbt "examples/runMain blammo.tracing.TracerExample"
  *
  * Try piping the output to jq for pretty printing: sbt "examples/runMain
  * blammo.tracing.TracerExample" 2>&1 | grep jsonMessage | jq '.jsonMessage'
  */
object TracerExample extends JsonConsoleLogger with Tracer {
  ...
  def main(args: Array[String]): Unit = {
    ...
    def errorHandlingExample(): Unit = {
      ...
      val failureResult = tracedTry(
        "payment.process",
        Map(
          "amount"   -> Json.JsonNumber(99.99),
          "currency" -> Json.JsonString("USD")
        )
      ) {
        println("[App] Processing payment...")
        Thread.sleep(50)
