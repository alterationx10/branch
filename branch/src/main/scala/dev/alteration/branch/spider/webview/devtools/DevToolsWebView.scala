...
object TimelineFilter {
  ...
  given JsonEncoder[TimelineFilter] = (filter: TimelineFilter) =>
    Json.JsonString(filter match {
  ...
  given JsonDecoder[TimelineFilter] = (json: Json) =>
    scala.util.Try {
      json match {
    ...
    encodeFunc = event => {
      ...
      event match {
        case SelectView(viewId) =>
          Json.obj(
            "type"   -> Json.JsonString("SelectView"),
            "viewId" -> Json.JsonString(viewId)
          )
        case SetFilter(filter)  =>
          Json.obj(
            "type"   -> Json.JsonString("SetFilter"),
            "filter" -> Json.JsonString(filter.toString)
          )
        case ClearTimeline      =>
          Json.obj("type" -> Json.JsonString("ClearTimeline"))
        case ClearDisconnected  =>
          Json.obj("type" -> Json.JsonString("ClearDisconnected"))
        ...
        clickButton(
          ...
          extraAttrs = Seq(
            style := filterButtonStyle(
              filter == TimelineFilter.OnlyStateChanges
            )
          )
        ),
        clickButton(
