...
object AdvancedComponents {
        ...
        tr()(
          columns.map { col =>
            th(
              ...
              if (col.sortable) attr("wv-click") := s"$onSort-${col.key}" else attr("data-key") := col.key
            )(
      ...
      tbody()(
        if (rows.isEmpty) {
          ...
          Html.Fragment(rows.map { row =>
            tr(style := "border-bottom: 1px solid #e2e8f0; transition: background 150ms;")(
              columns.map { col =>
      ...
      div(
        ...
        div(
          ...
          div(style := "padding: 20px; border-bottom: 1px solid #e2e8f0; display: flex; justify-content: space-between; align-items: center;")(
            h2(style := "margin: 0; font-size: 1.5rem; color: #2d3748;")(Html.Text(title)),
            button(
    ...
    div(style := "border-bottom: 2px solid #e2e8f0;")(
      div(style := "display: flex; gap: 4px;")(
        tabs.map { case (id, label) =>
          ...
          button(
            style := s"padding: 12px 24px; border: none; background: ${if (isActive) "white" else "transparent"}; color: ${if (isActive) Colors.primary else "#718096"}; border-bottom: ${if (isActive) s"3px solid ${Colors.primary}" else "3px solid transparent"}; cursor: pointer; font-size: 1rem; font-weight: ${if (isActive) "600" else "normal"}; transition: all 150ms;",
  ...
  def accordionItem(
    ...
    div(style := "border: 1px solid #e2e8f0; border-radius: 8px; margin-bottom: 8px; overflow: hidden;")(
      // Header
      div(
        style := s"padding: 16px; background: ${if (isOpen) "#f7fafc" else "white"}; cursor: pointer; display: flex; justify-content: space-between; align-items: center;",
  ...
  def card(
    ...
    div(style := s"background: white; border: 1px solid $borderColor; border-radius: 8px; box-shadow: ${Shadows.sm}; overflow: hidden;")(
      title match {
        case Some(t) =>
          div(style := "padding: 16px 20px; border-bottom: 1px solid #e2e8f0; background: #f7fafc;")(
            h3(style := "margin: 0; font-size: 1.25rem; color: #2d3748;")(Html.Text(t))
          )
        case None => Html.Empty
      },
      div(style := "padding: 20px;")(content),
      footer match {
        case Some(f) =>
          div(style := "padding: 16px 20px; border-top: 1px solid #e2e8f0; background: #f7fafc;")(f)
        case None => Html.Empty
      }
    )
  }
  ...
  def badge(text: String, variant: String = "default"): Html = {
    ...
    span(style := s"display: inline-block; padding: 4px 12px; background: $bg; color: $color; border-radius: ${Radius.full}; font-size: 0.875rem; font-weight: 600;")(
      Html.Text(text)
    )
  }
  ...
  def alert(
    ...
    div(style := s"padding: 16px; background: $bg; border: 1px solid $border; border-radius: 8px; color: $text; display: flex; justify-content: space-between; align-items: start; gap: 12px;")(
  ...
  def progressBar(
    ...
    div()(
      label match {
        case Some(l) =>
          div(style := "margin-bottom: 8px; font-size: 0.875rem; color: #4a5568; display: flex; justify-content: space-between;")(
            Html.Text(l),
            Html.Text(s"$clampedValue%")
          )
        case None => Html.Empty
      },
      div(style := s"width: 100%; height: 24px; background: #e2e8f0; border-radius: ${Radius.full}; overflow: hidden;")(
        div(style := s"height: 100%; width: $clampedValue%; background: $color; transition: width 300ms ease; display: flex; align-items: center; justify-content: center; color: white; font-size: 0.75rem; font-weight: 600;")(
  ...
  def pagination(
    ...
    div(style := "display: flex; gap: 4px; justify-content: center; align-items: center;")(
      Html.Fragment(List(
        // Previous button
        button(
          style := s"padding: 8px 12px; border: 1px solid #e2e8f0; background: white; border-radius: 6px; cursor: ${if (currentPage <= 1) "not-allowed" else "pointer"}; color: ${if (currentPage <= 1) "#a0aec0" else Colors.primary};",
          attr("wv-click") := s"$onPageChange-${math.max(1, currentPage - 1)}",
          if (currentPage <= 1) attr("disabled") := "true" else attr("data-enabled") := "true"
        )(Html.Text("← Prev"))
      ) ++
      // Page numbers (show current and 2 on each side)
      (math.max(1, currentPage - 2) to math.min(totalPages, currentPage + 2)).map { page =>
        button(
          style := s"padding: 8px 12px; border: 1px solid ${if (page == currentPage) Colors.primary else "#e2e8f0"}; background: ${if (page == currentPage) Colors.primary else "white"}; color: ${if (page == currentPage) "white" else "#2d3748"}; border-radius: 6px; cursor: pointer; font-weight: ${if (page == currentPage) "600" else "normal"};",
      ...
      List(
        ...
        button(
          ...
          if (currentPage >= totalPages) attr("disabled") := "true" else attr("data-enabled") := "true"
        )(Html.Text("Next →"))
      ))
    )
  }
