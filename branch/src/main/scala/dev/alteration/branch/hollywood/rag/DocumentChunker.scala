...
object DocumentChunker {
  ...
  def chunk(text: String, config: ChunkConfig): ChunkResult = {
  ...
  private def chunkBySentence(text: String, config: ChunkConfig): List[Chunk] = {
    val sentences = splitIntoSentences(text)
    groupByCount(sentences, config.chunkSize, config.overlap)
  }

  // Split by paragraph boundaries
  private def chunkByParagraph(text: String, config: ChunkConfig): List[Chunk] = {
    val paragraphs = text.split("\\n\\s*\\n").toList.map(_.trim).filter(_.nonEmpty)
    groupByCount(paragraphs, config.chunkSize, config.overlap)
  }

  // Split by character count
  private def chunkByCharacter(text: String, config: ChunkConfig): List[Chunk] = {
