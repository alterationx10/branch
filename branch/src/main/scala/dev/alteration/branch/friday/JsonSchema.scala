...
object Schema {
  ...
  case class StringSchema(description: Option[String] = None)  extends Schema
  case class NumberSchema(description: Option[String] = None)  extends Schema
  case class IntegerSchema(description: Option[String] = None) extends Schema
  case class BooleanSchema(description: Option[String] = None) extends Schema
...
object JsonSchema {
  ...
  given [A](using inner: JsonSchema[A]): JsonSchema[Option[A]] with {
    def schema: Schema = inner.schema
  }

  // Collection types
  given [A](using inner: JsonSchema[A]): JsonSchema[List[A]] with {
  ...
  given [A](using
      inner: JsonSchema[A]
  ): JsonSchema[IndexedSeq[A]] with {
    def schema: Schema = ArraySchema(inner.schema)
  }

  given [A](using inner: JsonSchema[A]): JsonSchema[Vector[A]] with {
    def schema: Schema = ArraySchema(inner.schema)
  }

  given [A](using inner: JsonSchema[A]): JsonSchema[Set[A]] with {
    def schema: Schema = ArraySchema(inner.schema)
  }
        ...
        Json.JsonObject(
          Map(
            ...
            "properties" -> Json.JsonObject(
              properties.view.mapValues(toJson).toMap
            ),
